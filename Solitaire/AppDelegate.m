...
- (BOOL)shouldAutorotateToInterfaceOrientation:(UIInterfaceOrientation)interfaceOrientation
{
	// iPhone only
	if( [[UIDevice currentDevice] userInterfaceIdiom] == UIUserInterfaceIdiomPhone )
		return UIDeviceOrientationPortrait == interfaceOrientation;
...
-(void) directorDidReshapeProjection:(CCDirector*)director
{
	if(director.runningScene == nil) {
        ...
        [[UserDefault sharedUserDefault] synchronize];
        [[AudioManager sharedAudioManager] load];
        [[IdentifieManager sharedIdentifieManager] test];
        
        [[HistoryManager sharedHistoryManager] load];
        [[PuzzleRecord sharedPuzzleRecord] load];
        [[LevelManager sharedLevelManager] load];
...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions
{
	...
	[sharedFileUtils setiPadRetinaDisplaySuffix:@"_iPad@2x"];	// Default on iPad RetinaDisplay is "-ipadhd"
...
-(void) applicationDidEnterBackground:(UIApplication*)application
{
    ...
    [[AudioManager sharedAudioManager] save];
    
    [[LevelManager sharedLevelManager] save];
	if( [navController_ visibleViewController] == director_ )
